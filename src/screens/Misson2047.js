...
const Mission2047 = ({
    navigation,
			...
			<MHeader navigation={navigation} title={"MISSION 2047"} />
      <View style={styles.pageWrapper}>
        {participateText.split('\n').map(x => <Text style={styles.text_16_600}>{x}</Text>)}
        <MButton mbStyle={styles.mbStyle} btnText={"Click here to participate"} onPress={() => alert("Click here to participate")} />
        {bankStatementText.split('\n').map(x => <Text style={styles.text_16_600}>{x}</Text>)}
        <MButton mbStyle={styles.mbStyle} btnText={"Download  Bank Statement"} onPress={() => alert("Download  Bank Statement")} />
      </View>
	</ScrollView>)
